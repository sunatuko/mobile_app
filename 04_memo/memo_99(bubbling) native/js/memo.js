...
const delLocalStorage = () => {
    ...
    // 一つを削除する
    // 2021/11/25 fukada add-str
    // クラス「.delItem」を持つ（将来の）子のいずれかでクリックイベントが発生すると、
    // そのクリックイベントは親にバブルアップする。
    // 親は、そのセレクター（'.delItem'）に一致する子からイベントが発生したかどうかを確認し、一致した場合、子に直接バインドされているかのように子に対してイベントを処理する。
    const table1 = document.getElementById('table1');
    table1.addEventListener('click', (e) => {
        if(e.target.classList.contains('delItem') === true){
            ...
            Swal.fire({
                title : "Memo app", //titleのスペルが間違ってたよ
                html : w_delete,
                type : "question",
                showCancelButton : true
            }).then(result => {
    ...
    //     parent = $(this).parent(); //クラス「.delItem」の親（td）を取得
    //     let key = $(parent).prev().prev().text(); // 親（td）の前のセル（左隣）を取得
    //     let value = $(parent).prev().text(); // 親（td）の前のセル（左隣）を取得
    //     let w_delete = `LocalStorageに[${key} ${value}]を削除しますか？`;
    //     Swal.fire({
    //         title : "Memo app", //titleのスペルが間違ってたよ
    //         html : w_delete,
